...
public sealed class GetProjectsServiceTests
{
    ...
    public async Task ExecuteAsync_WithCombinedFilters_MapsCriteriaAndClientSummary()
    {
        ...
        var result =
            await context.GetProjectsService.ExecuteAsync(
        ...
        Assert.False(captured.PendingAttentionOnly);
    ...
    [Fact]
    public async Task ExecuteAsync_WithPendingAttention_MapsPendingAttentionCriteria()
    {
        var context = new AdministrationTestContext();
        ProjectSearchCriteria? captured = null;
        ...
        Assert.True(result.IsSuccess);
        Assert.NotNull(captured);
        Assert.True(captured.PendingAttentionOnly);
    }
